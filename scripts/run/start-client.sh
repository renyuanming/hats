...
func() {
    ...
    if [ $mode == "mlsm" ]; then
        ...
        echo "Disable multiple LSM-tree"
        bin/cqlsh $coordinator -e "create keyspace ycsbraw WITH REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor': $replication_factor };
        USE ycsbraw;
