...
import static org.iq80.twoLayerLog.util.SizeOf.SIZE_OF_BYTE;
import static org.iq80.twoLayerLog.util.SizeOf.SIZE_OF_INT;
import static org.iq80.twoLayerLog.util.SizeOf.SIZE_OF_LONG;
import static org.iq80.twoLayerLog.util.SizeOf.SIZE_OF_SHORT;
        ...
        implements Comparable<Slice>, Serializable
{
    ...
    public static long bytesToLong(byte[] input, int offset, int length) { 
        // 灏哹yte[] 灏佽涓� ByteBuffer 
        ByteBuffer buffer = ByteBuffer.wrap(input, offset, length);
        /*if(littleEndian){
            // ByteBuffer.order(ByteOrder) 鏂规硶鎸囧畾瀛楄妭搴�,鍗冲ぇ灏忕妯″紡(BIG_ENDIAN/LITTLE_ENDIAN)
            // ByteBuffer 榛樿涓哄ぇ绔�(BIG_ENDIAN)妯″紡 
    ...
    public int compareTo(Slice that)
    {
        ...
        //String thisStr = new String(this.data, 0, length);
        //String thatStr = new String(that.data, 0, that.length);
        String thisStr = new String(this.data, this.getRawOffset(), length);
        String thatStr = new String(that.data, that.getRawOffset(), that.length);
        //String str = "thisStr:"+ thisStr + " , thatStr:"+thatStr;
        //StorageService.instance.printInfo(str);
        Long thisLong = Long.valueOf(thisStr);
        Long thatLong = Long.valueOf(thatStr);
        Long distance =  thisLong - thatLong;
        //String str1 = "thisLong:"+ thisLong + " , thatLong:"+thatLong+", distance:"+ distance+", flag:"+ (distance.longValue() == 0L);
        //StorageService.instance.printInfo(str1);

        if(distance.longValue() == 0L){ 
            return 0;        
        }else if(distance.longValue() > 0L){
            return 1;         
        }else {
            return -1;
        }
