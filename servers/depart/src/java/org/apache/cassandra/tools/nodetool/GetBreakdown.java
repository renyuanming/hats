...
public class GetBreakdown extends NodeToolCmd
{
    ...
    public void execute(NodeProbe probe)
    {
        ...
        for(String keyspace : tablesList.keys().elementSet())
        {
            ...
            System.out.println(format("%-10s%19s%19s%19s%19s%19s%19s%19s%19s%19s",
            "Keypsace", "Table", "Read Latency", "Read Count", "Write Latency", "Write Count", "Coord Read", "Coord ReadCnt", "Coord Scan", "Coord ScanCnt"));
            for(String table : tablesList.get(keyspace))
            { 
                
                System.out.println(format("%-10s%19s%19.2f%19s%19.2f%19s%19.2f%19s%19.2f%19s",
            ...
            System.out.println(format("Local read latency: %.2f", averageLocalReadLatency));
            // System.out.println(format("The cost of replica selection for %s: %f", averageCoordiantorReadLatency - averageLocalReadLatency));
            System.out.println(format("Local read count: %d", totalReadCount));
            System.out.println(format("Local write latency: %.2f", averageLocalWriteLatency));
            System.out.println(format("Local write count: %d", totalWriteCount));
            System.out.println(format("Coordinator read latency: %.2f", averageCoordiantorReadLatency));
            System.out.println(format("Coordinator read count: %d", totalCoordinatorReadCount));
            System.out.println(format("Coordinator scan latency: %.2f", averageCoordiantorScanLatency));
            System.out.println(format("Coordinator scan count: %d", totalCoordinatorScanCount));
            System.out.println();
        ...
        System.out.println("Print the average messaging queue wait latency for each message type:");
        // for (String messageType : messageTypes)
        // {
        //     System.out.println(format("Wait latency for %s: %f", messageType, probe.getMessagingQueueWaitMetrics(messageType).getMean()));
        // }
        System.out.println();

        // Get the network operations latency
        String[] networkOperations = {"Read", "Write"};
        System.out.println("Print the average network operations latency for each operation type:");
        for (String operation : networkOperations)
        {
            System.out.println(format("Network latency for %s: %f", operation, probe.getProxyMetric(operation).getMean()));
        }
