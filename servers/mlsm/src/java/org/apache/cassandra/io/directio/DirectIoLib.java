 ...
 */
package org.apache.cassandra.io.directio;
...
public class DirectIoLib {
    ...
    static {
    	binit = false;
        try {
            if (!Platform.isLinux()) { // TODO (smacke): test on other *nix variants
                ...
                if (binit) {
                	Native.register(Platform.C_LIBRARY_NAME); // get access to open(), pread(), etc
                } else {
